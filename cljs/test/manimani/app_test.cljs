...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates every fact the Svelte scaffold held"
    ...
    (is (= "Ai etzhayyim Project Manimani" @(rf/subscribe [:app/title])))
    (is (= "etzhayyim-project-manimani" @(rf/subscribe [:app/name])))
    (is (= "etzhayyim-project-manimani" @(rf/subscribe [:app/project])))
